...
data class ModelRequest(
    /** Full assembled prompt — used by LiteRtBackend (multimodal path). */
    val prompt: String,
    val screenshotPath: String,
    /**
     * The cacheable, static portion of the prompt: system role, decision rules,
     * JSON schema, and examples. Rarely changes between steps.
     * Used by LlamaCppBackend to fill the KV cache once and reuse it.
     */
    val staticPrefix: String,
    /**
     * The per-step, dynamic portion: goal, current app, action history, injections.
     * Appended after the KV-cached prefix for each generate() call.
     */
...
class ModelInputBuilder {
    fun build(
        ...
        val safeGoal = goal.sanitizePromptValue()
        val safeCurrentAppName = currentAppName.sanitizePromptValue()
        val stepsDone = if (history.records.isEmpty()) {
            ...
            history.records.joinToString("\n") { record ->
                ...
                val actionDetails = when (action.type) {
                    UiActionType.OPEN_APP -> action.appName.orEmpty().sanitizePromptValue()
                    UiActionType.TYPE_TEXT -> "text: '${action.text.orEmpty().sanitizePromptValue()}'"
                    UiActionType.CLICK -> "reason: ${action.reason.sanitizePromptValue()}"
                    else -> "reason: ${action.reason.sanitizePromptValue()}"
                }
                val resultMessage = record.executionResult.message.sanitizePromptValue()
                
                val resultStr = if (record.executionResult.success) {
                    "success"
                } else {
                    if (action.type == UiActionType.OPEN_APP) {
                        "failure (Try different approach. Try different name or go to home screen, scroll up and use search bar. Error: $resultMessage)"
                    } else {
                        "failure (Try different approach. Error: $resultMessage)"
                    }
                }
        ...
        val prompt = """
                ...
                GOAL: $safeGoal
                Current App: $safeCurrentAppName
        ...
        DbgLog.d(
            "Prompt built goalChars=${goal.length} step=$stepNumber/$maxSteps " +
                ...
                "currentAppName=$safeCurrentAppName currentPackageName=${currentPackageName ?: "unknown"} " +
        ...
        return ModelRequest(
            ...
            staticPrefix  = buildStaticPrefix(),
            dynamicPrompt = buildDynamicPrompt(
                goal                       = safeGoal,
                currentAppName             = safeCurrentAppName,
                detectedTargetAppContextStr = detectedTargetAppContextStr,
                stepsDone                  = stepsDone,
    ...
    /**
     * The static (cacheable) part of the prompt.
     * Contains the system role declaration, all decision rules, output schema,
     * and the few-shot examples. This text NEVER changes between steps within a
     * session (or even across sessions), so llama.cpp can keep it in the KV cache
     * indefinitely until the model is reloaded.
     */
    private fun buildStaticPrefix(): String = """
                Act like UIActionAgent, an on-device Android UI automation agent.
    ...
    /**
     * The dynamic (per-step) part of the prompt.
     * Appended to the static prefix for each inference call.
     */
    private fun buildDynamicPrompt(
        goal: String,
        currentAppName: String,
        detectedTargetAppContextStr: String,
        stepsDone: String,
        injectionStr: String,
        errorWarning: String,
    ): String = """
